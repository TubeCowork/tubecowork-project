...
export const getObjectId = withTryCatch(async (id: string): Promise<mongoose.Types.ObjectId> => {
...
export const fetchUserDetails = withTryCatch(async (
    userEmail: string
...
export const fetchChannelDetails = withTryCatch(async (
    channelid: string
): Promise<YoutubeChannelType> => {
    await connectToDB();
    const channelObjectId = await getObjectId(channelid);
    const channel = await YoutubeChannelModel.findById(channelObjectId).populate({
        path: "editors",
        model: UserModel,
    });
    ...
    const editors = channel?.editors?.map(
        (user: IUser) => {
            return {
                id: String(user._id),
                name: user.name,
    ...
    const videos = channel?.videos?.map(
        (video: IVideo) => {
            return {
                id: String(video._id),
