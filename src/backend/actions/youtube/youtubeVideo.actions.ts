...
export const createYoutubeChannel = async (videoDetails: YoutubeVideoUploadDataType, channelId: ObjectId, userid: ObjectId): Promise<IVideo> => {
    try {
        const { title, description, tags, videoFile, thumbnailFile } = videoDetails;
        ...
        const { videoYoutubeId, videoURL, thumbnailURL } = await uploadVideoUnlisted(videoDetails, channel);
...
export const approveUploadedVideo = async (channelId: ObjectId, videoId: ObjectId) => {
    try {
        const channel = await YoutubeChannelModel.findById(channelId);
