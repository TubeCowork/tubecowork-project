...
): Promise<{ videoYoutubeId: string, videoURL: string; thumbnailURL: string }> => {
    try {
...
export const makeVideoPublic = async (videoYoutubeId: string, channel: IYoutubeChannel): Promise<boolean> => {
    try {
        const youtube = await authticateYoutubeWithChannel(channel);
