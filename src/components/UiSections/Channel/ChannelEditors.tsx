...
function ChannelEditors({ addEditor, allEditors }: ChannelEditorsType) {
    ...
    return (
        <div>
            <Button text='Add New Editor' onClick={() => {
            ...
            <PopUpModal isOpen={showAddEditorPopup} closeModal={() => {
                setShowAddEditorPopup(false);
            }}>
                <div className="">
                    <h1>Add new Editor</h1>
                    <form onSubmit={addEditorToChannel} className='flex_center flex-col'>
                        <NormalInput placeholder='Enter User Email' name='email' id='email' />
                        <Button text='Add Editor' type='submit' loading={adding} />
                    </form>
                    {FormMsg &&
                        <p>{FormMsg}</p>
                    }
